...
if($_SERVER['REQUEST_METHOD']=='POST')
{
    ...
    while($tbl = mysqli_fetch_array($retorno)){
        ...
        if($cont == 1){
            print"<script>window.alert('PRODUTO JA CADASTRADO);</script>";
        }
        else
        {
        $sql = "INSERT INTO produtos (pro_nome, pro_descricao, pro_quantidade, pro_custo, pro_preco, pro_ativo, imagem1)
        VALUE ('$nome', '$descricao', '$quantidade', '$custo', '$preco', 's', '$imagem_base64')";
...
<body>
    <div>
        ...
        <!--ESTRUTURA DA PAGINA -->
    <form action="cadastraprodutos.php" method="post" enctype="multipart/form-data">
            <label>NOME DO PRODUTO</label>
            <input type="text" name="nome" id="nome">
            <br>

            <label>DESCRIÇÃO</label>
            <textarea name="descricao" id="descricao" rows="4" resize="nome"></textarea>
            <br>
